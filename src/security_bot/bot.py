...
def _shift_message_entities(message: Message, payload_start_offset: int) -> list[dict[str, object]]:
    shifted: list[dict[str, object]] = []
    for entity in message.entities or []:
        if entity.type not in WARNING_ENTITY_TYPES:
            continue
...
def _warning_entities(context: ContextTypes.DEFAULT_TYPE, settings) -> list[MessageEntity] | None:
    ...
    entities = [
        entity
        for entity in (MessageEntity.de_json(entity_data, context.bot) for entity_data in settings.warning_entities)
...
async def warningfreq(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        return
    if seconds < MIN_WARNING_FREQ_SECONDS:
        await message.reply_text(f"Frequency must be at least {MIN_WARNING_FREQ_SECONDS} seconds.")
        return
    settings = _store(context).chat(chat.id)
...
async def _send_configured_warning(context: ContextTypes.DEFAULT_TYPE, chat_id: int) -> Message:
    settings = _store(context).chat(chat_id)
...
def _schedule_warning_job(context: ContextTypes.DEFAULT_TYPE, chat_id: int) -> None:
    ...
    if not settings.warning_enabled:
        return
    interval = max(settings.warning_freq_seconds, MIN_WARNING_FREQ_SECONDS)
